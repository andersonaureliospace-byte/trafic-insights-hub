...
export async function GET() {
  try {
    ...
    const { statuses, sendError } = await checkLowInvestment(supabase, user.id, token, { send: false });
...
export async function POST() {
  try {
    const { supabase, user } = await requireUser();
    const token = await getUserMetaToken(supabase, user.id);
    const { statuses, sendError } = await checkLowInvestment(supabase, user.id, token, { send: true });
    return NextResponse.json({ statuses, sendError });
  } catch (e) {
    return NextResponse.json({ error: (e as Error).message }, { status: 400 });
  }
