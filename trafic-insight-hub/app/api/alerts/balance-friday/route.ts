...
import { checkFridayLowBalances } from "@/lib/alerts/balance";
...
export async function GET() {
  try {
    ...
    const { statuses, sendError } = await checkFridayLowBalances(supabase, user.id, token, { send: false });
...
export async function POST() {
  try {
    const { supabase, user } = await requireUser();
    const token = await getUserMetaToken(supabase, user.id);
    const { statuses, sendError } = await checkFridayLowBalances(supabase, user.id, token, {
      send: true,
      bypassCooldown: true,
    });
    return NextResponse.json({ statuses, sendError });
